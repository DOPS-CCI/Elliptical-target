...
namespace CCIUtilities
{
    ...
    */
    ...
    {
        private static readonly double num_Ln2 = 0.69314718055994530941; // ln(2)
        private static readonly double PI2 = Math.PI * Math.PI;
        private static readonly double PI4 = PI2 * PI2;
        private static readonly int NFACT = 20;
        private static readonly int MFACT = 30;
        ...
        private static readonly int NEXACT = 140;
        private static readonly int NKOLMO = 100000;

        // For the Durbin matrix algorithm
        private static readonly double NORM = 1.0e140;
        private static readonly double INORM = 1.0e-140;
        private static readonly int LOGNORM = 140;
        ...
        private static readonly double[] LnFactorial = {
            0D,
            0D,
            0.6931471805599453,
            1.791759469228055,
        ...
        private static double KSPlusbarUpper(int n, double x)
        {
            ...
            {
                q = (double)j / n + x;
                term = LogCom + (j - 1) * Math.Log(q) + (n - j) * Math.Log(1D - q);
                t = Math.Exp(term);
                Sum += t;
        ...
        private static double Pelz(int n, double x)
        {
            ...
            int JMAX = 20;
            double EPS = 1.0e-10;
            double C = 2.506628274631001;         // sqrt(2*Pi)
            double C2 = 1.2533141373155001;       // sqrt(Pi/2)
            double RACN = Math.Sqrt((double)n);
            double z = RACN * x;
            double z2 = z * z;
            double z4 = z2 * z2;
            double z6 = z4 * z2;
            double w = PI2 / (2.0 * z * z);
            double ti, term, tom;
            double sum;
            int j;
